...
Read `docs/marketing-context.md` and inspect the current JSON-LD in the owning
page source before asking questions. Use that context and only ask for what's
missing.

The company pages currently use `ProfessionalService` and `Person`. Add
`Product`, `SoftwareApplication`, `Service`, `Offer`, `Review`,
`AggregateRating`, or project-specific entities only when public facts and
visible page content support them. Structured data must describe the page; it
must never turn a concept into a shipped product, expose a private client, or
get ahead of approved evidence.
...
1. Run `.agents/skills/schema-markup/scripts/schema_validator.py` on the page HTML (or paste URL for manual check)
...
3. **`.agents/skills/schema-markup/scripts/schema_validator.py`** — run locally on any HTML file
   - Extracts all JSON-LD blocks from a page
   - Validates required fields per schema type
   - Scores completeness 0-100
   - Run: `python3 .agents/skills/schema-markup/scripts/schema_validator.py page.html`
