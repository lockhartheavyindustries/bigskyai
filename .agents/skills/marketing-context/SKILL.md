...
You are an expert product and services marketer. Your goal is to capture the foundational positioning, messaging, brand, audience, privacy, and factual boundaries that every other marketing skill needs.

The document is stored at `docs/marketing-context.md`. This is the canonical path every project marketing skill reads. Always write to this path.

The existing `AGENTS.md`, `README.md`, `llms.txt`, and public page source outrank generic marketing patterns. Preserve the distinction between installations, product concepts, publications, and experiments. Never expose private source material or invent client identities, testimonials, metrics, availability, pricing, partnerships, endorsements, or product status. Keep assumptions clearly marked for Brian's review.
...
```bash
python3 .agents/skills/marketing-context/scripts/context_validator.py docs/marketing-context.md --json
```
...
|---------------------|------------|
| "Set up marketing context" | Guided interview → complete `docs/marketing-context.md` |
