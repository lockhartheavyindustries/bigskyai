...
Read `docs/marketing-context.md`, `AGENTS.md`, and `README.md` before auditing. Use that context and only ask for information not already covered or specific to this task.

For Big Sky AI, verify repository source, crawler-visible production HTML,
Search Console data, and real-user performance separately. Generated pages
must be traced to their source templates. Script scores are directional
heuristics, not ranking factors or proof of a problem. Never expose private
material or lengthen clear, approved copy merely to satisfy a word-count or
...
|---|---|---|
| On-page checker | `python3 .agents/skills/seo-audit/scripts/seo_checker.py --file page.html` (or `--url https://...`; `--json`) | Directional check of title/meta/headings/links/images |
| Health scorer | `python3 .agents/skills/seo-audit/scripts/seo_health_scorer.py --checks checks.json --industry local --json` | Weighted heuristic across 7 categories |
...
- **marketing-context** — WHEN: always read `docs/marketing-context.md` first to avoid redundant questions and protect privacy, factual, voice, and project-status boundaries.
