...
Read `docs/marketing-context.md`, `AGENTS.md`, and `README.md` before editing.
Preserve Big Sky AI's clear, experienced, practical voice and the owning
project's status. Do not expose private material, turn a concept into a shipped
product, invent client proof, or introduce generic AI hype, urgency, pricing,
availability, endorsements, or unsupported performance claims.

The readability and AI-pattern scripts are highlighting tools, not style
judges. Never flatten purposeful cadence, humor, or technical precision solely
to improve a mechanical score.
...
**Process:**
1. Score the draft mechanically first: `python3 .agents/skills/copy-editing/scripts/readability_scorer.py --file draft.md` (Flesch score, passive-voice %, filler-word count; add `--json` for pipelines). Anything it flags is your starting highlight list, not an automatic rewrite order.
...
```bash
python3 .agents/skills/copy-editing/scripts/ai_content_detector.py draft.md --json   # no arg = --demo mode
```
